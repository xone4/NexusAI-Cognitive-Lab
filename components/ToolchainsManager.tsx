...
const ToolchainItem: React.FC<{
    ...
    const handleDelete = () => {
        if (window.confirm(t('toolchains.deleteConfirm', { name: toolchain.name }))) {
    ...
    return (
        <div className="bg-nexus-dark/50 p-4 rounded-xl border border-nexus-surface/50 transition-all duration-300 hover:border-nexus-primary/70 animate-spawn-in">
            ...
            <div className="mt-3 pt-3 border-t border-nexus-surface/30">
                 <h5 className="text-xs font-semibold text-nexus-primary uppercase tracking-wider mb-2">{t('toolchains.sequence')}</h5>
                        ...
                        return (
                            <React.Fragment key={tid}>
                                <span className="bg-nexus-surface px-2 py-1 rounded-full text-nexus-text font-mono">
                                    {tool?.name || t('toolchains.unknownTool')}
                                </span>
                                {index < toolchain.toolIds.length - 1 && <ArrowRightIcon className="w-5 h-5 text-nexus-secondary" />}
                            </React.Fragment>
                        );
                    }) : <p className="text-xs text-nexus-text-muted">{t('toolchains.noToolsInChain')}</p>}
                 </div>
            </div>
        </div>
    );
...
const ToolchainsManager: React.FC<ToolchainsManagerProps> = ({ allTools, toolchains, isThinking, onCreate, onUpdate, onDelete }) => {
    ...
    return (
        <>
            ...
            <DashboardCard title={t('toolchains.managementTitle')} icon={<LinkIcon />} className="md:col-span-2 xl:col-span-3">
                <div className="flex flex-col md:flex-row gap-4 items-center mb-4 pb-4 border-b border-nexus-surface/50">
                    <div className="flex-grow">
                        <h3 className="text-lg font-semibold text-nexus-text">{t('toolchains.workflowsTitle')}</h3>
                        <p className="text-sm text-nexus-text-muted">{t('toolchains.workflowsDesc')}</p>
                    </div>
                     <button
                    ...
                    {toolchains.length > 0 ? (
                        toolchains.map(tc => (
                         ...
                         <div className="text-center py-8 text-nexus-text-muted">
                            ...
                            <h3 className="text-lg font-semibold">{t('toolchains.noToolchains')}</h3>
                            <p className="text-sm">{t('toolchains.noToolchainsHint')}</p>
                        </div>
                    )}
                </div>
            </DashboardCard>

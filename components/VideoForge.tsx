...
const VideoForge: React.FC = () => {
    ...
    if (!hasApiKey) {
        return (
            <DashboardCard title={t('videoForge.apiKeyRequired')} icon={<FilmIcon/>}>
                <div className="text-center p-4">
                    <p className="text-nexus-text-muted mb-4">{t('videoForge.apiKeyDescription')}</p>
    ...
    return (
        <div className="space-y-6 max-w-4xl mx-auto">
            <DashboardCard title={t('videoForge.title')} icon={<FilmIcon />}>
                <div className="space-y-4">
                    <textarea
                        value={prompt}
                        onChange={(e) => setPrompt(e.target.value)}
                        placeholder={t('videoForge.promptPlaceholder')}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label className="text-sm font-medium text-nexus-text-muted">{t('videoForge.aspectRatio')}</label>
                            <select value={aspectRatio} onChange={e => setAspectRatio(e.target.value as '16:9' | '9:16')} disabled={isGenerating} className="w-full mt-1 config-input">
                                <option value="16:9">{t('videoForge.landscape')}</option>
                                <option value="9:16">{t('videoForge.portrait')}</option>
                            </select>
                        </div>
                         <div>
                            <label className="text-sm font-medium text-nexus-text-muted">{t('videoForge.resolution')}</label>
                    ...
                    <button onClick={handleGenerate} disabled={isGenerating || !prompt.trim()} className="w-full flex items-center justify-center gap-2 bg-nexus-primary/90 text-nexus-dark font-bold py-3 px-4 rounded-full border border-nexus-primary/80 hover:bg-nexus-secondary transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed">
                        ...
                        {isGenerating ? t('videoForge.generating') : t('videoForge.generate')}
                    </button>
                </div>
            </DashboardCard>
            
            {isGenerating && (
                <DashboardCard title={t('videoForge.generationStatus')} icon={<div className="w-5 h-5 relative"><div className="nexus-loader"></div></div>}>
                    <div className="text-center p-4">
                        <p className="text-lg font-semibold text-nexus-text animate-pulse">{statusMessage}</p>
                        <p className="text-sm text-nexus-text-muted mt-2">{t('videoForge.patience')}</p>
                    </div>
                </DashboardCard>
            )}

            {error && (
                <DashboardCard title={t('videoForge.errorTitle')} icon={<XCircleIcon/>}>
            ...
            {videoUrl && (
                <DashboardCard title={t('videoForge.result')} icon={<CheckCircleIcon/>}>
                    <div className="space-y-4">

...
const ToolCard: React.FC<Omit<MentalToolsLabProps, 'tools' | 'toolchains' | 'onForgeTool' | 'onCreateToolchain' | 'onUpdateToolchain' | 'onDeleteToolchain'> & { tool: MentalTool; onSelectModify: (tool: MentalTool) => void; }> = memo(({ tool, isThinking, onToggleStatus, onOptimizeTool, onArchiveTool, onDecommissionTool, onSelectModify }) => {
    ...
    return (
        <DashboardCard 
            title={tool.name} 
            icon={<BeakerIcon className={style.text} />} 
            className={`border-l-4 ${style.border} ${style.anim || ''} transition-all duration-300 flex flex-col`}
        >
            <div className="flex-grow space-y-3">
                <div className="flex justify-between items-start">
                    <p className={`text-sm font-bold ${style.text}`}>Status: {tool.status}</p>
                    <p className="text-sm text-nexus-text-muted">{`v${tool.version.toFixed(1)} | Comp: ${tool.complexity}`}</p>
                </div>
                <p className="text-sm text-nexus-text-muted italic flex-grow">"{tool.description}"</p>
                <div>
                    <h4 className="text-xs font-semibold text-nexus-primary uppercase tracking-wider mb-1">Tags</h4>
                    <div className="flex flex-wrap gap-1.5">
                        {tool.tags.map(tag => (
                            <span key={tag} className="text-xs bg-nexus-dark px-2 py-0.5 rounded-full text-nexus-secondary font-mono">{tag}</span>
                        ))}
                         {tool.tags.length === 0 && <span className="text-xs text-nexus-text-muted font-mono">No tags assigned.</span>}
                    </div>
                </div>
            </div>
            ...
            <div className="mt-4 pt-3 border-t border-nexus-surface/30">
                 <Menu as="div" className="relative inline-block text-left w-full">
                    ...
                    <Transition as={Fragment} enter="transition ease-out duration-100" enterFrom="transform opacity-0 scale-95" enterTo="transform opacity-100 scale-100" leave="transition ease-in duration-75" leaveFrom="transform opacity-100 scale-100" leaveTo="transform opacity-0 scale-95">
                        <Menu.Items className="absolute bottom-full mb-2 w-56 origin-bottom-left rounded-md bg-nexus-bg shadow-lg ring-1 ring-nexus-surface ring-opacity-50 focus:outline-none z-10">
                            <div className="py-1">
                                ...
                                <Menu.Item>
                                {({ active }) => (
                                    <button onClick={() => onSelectModify(tool)} className={`${active ? 'bg-nexus-surface text-white' : 'text-nexus-text-muted'} group flex w-full items-center rounded-md px-2 py-2 text-sm`}>
...
const MentalToolsLab: React.FC<MentalToolsLabProps> = (props) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Control Header */}
      <DashboardCard title="Mental Tools Laboratory" icon={<BeakerIcon />}>
        <div className="flex flex-col md:flex-row gap-4 items-center">
            <div className="flex-grow flex gap-4">
                {/* Filter */}
                <div>
                    <label htmlFor="filter-status" className="text-xs font-semibold text-nexus-text-muted">Filter by Status</label>
                    <select id="filter-status" value={filterStatus} onChange={e => setFilterStatus(e.target.value as FilterStatus)} className="w-full mt-1 block rounded-md border-0 py-1.5 pl-3 pr-10 bg-nexus-bg text-nexus-text ring-1 ring-inset ring-nexus-surface focus:ring-2 focus:ring-nexus-primary sm:text-sm sm:leading-6">
                        <option>All</option>
                        <option>Idle</option>
                ...
                {/* Sort */}
                 <div>
                    <label htmlFor="sort-key" className="text-xs font-semibold text-nexus-text-muted">Sort By</label>
                    <select id="sort-key" value={sortKey} onChange={e => setSortKey(e.target.value as SortKey)} className="w-full mt-1 block rounded-md border-0 py-1.5 pl-3 pr-10 bg-nexus-bg text-nexus-text ring-1 ring-inset ring-nexus-surface focus:ring-2 focus:ring-nexus-primary sm:text-sm sm:leading-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        ...
        {filteredAndSortedTools.length === 0 && (
            <div className="md:col-span-2 xl:col-span-3 text-center text-nexus-text-muted py-10">
                ...
                <h3 className="text-lg font-semibold">No Mental Tools Match Criteria</h3>
                <p className="text-sm">Try adjusting filters or forging a new tool.</p>
            </div>
        )}
      </div>
